...
class PGDAttack(AttackABC):
    ...
    def _attack(self, n_perturbations, **kwargs):
        ...
        victim_model = self.attacked_model
        victim_model.eval()
        ...
        for t in tqdm(range(self.epochs)):
            mod_adj = self.get_modified_adj(ori_adj)
            mod_adj = utils.normalize_adj_tensor(mod_adj)
            # mod_adj = SparseTensor.from_dense(mod_adj)
            output = victim_model(ori_features, mod_adj)
            loss = self.calculate_loss(output[mask], self.labels[mask])
        ...
        self.random_sample(ori_adj, ori_features, self.labels, mask, n_perturbations)
        self.adj_adversary = SparseTensor.from_dense(self.get_modified_adj(ori_adj).detach()).coalesce().detach()
    ...
    def random_sample(self, ori_adj, ori_features, labels, idx_train, n_perturbations):
        ...
        with torch.no_grad():
            ...
            for i in range(K):
                ...
                # mod_adj = SparseTensor.from_dense(mod_adj)
                output = victim_model(ori_features, mod_adj)
                loss = self.calculate_loss(output[idx_train], labels[idx_train])
    ...
    def get_modified_adj(self, ori_adj):
        ...
        tril_indices = torch.tril_indices(row=self.nnodes, col=self.nnodes, offset=-1, device=self.device)
