...
# class Mettack(AttackABC):
#
#     def __init__(self,
#                  attack_config, pyg_data,
#                  model, device, logger, **kwargs):
#         super(Mettack, self).__init__(attack_config, pyg_data, model, device, logger)
#
#         self.lambda_ = attack_config['lambda_']
#         self.undirected = attack_config['undirected']
#         self.inner_epochs = attack_config['inner_epochs']
#         self.inner_lr = attack_config['inner_lr']
#         self.inner_momentum = attack_config['inner_momentum']
#         self.ll_constraint = attack_config['ll_constraint']
#         self.ll_cutoff = attack_config['ll_cutoff']
#
#         # self.modified_adj = None
#         self.nnodes = pyg_data.num_nodes
#
#         self.ori_adj = pyg_data.adj_t.to_dense().to(self.device)
#         self.adj_changes = nn.Parameter(torch.FloatTensor(self.nnodes, self.nnodes).to(self.device))
#         self.adj_changes.data.fill_(0.0)
#
#         config = self.attacked_model.config
#         surrogate_type = type(self.attacked_model)
#         self.surrogate = surrogate_type(
#             config, pyg_data, device, logger,
#             with_relu=False, with_bias=True, dense_conv=True).to(self.device)
#
#         self.train_mask = pyg_data.train_mask
#         self.unlabeled_mask = (pyg_data.val_mask | pyg_data.test_mask)
#         self.labels = pyg_data.y.to(self.device)
#
...
#         modified_adj = adj_changes_square + self.ori_adj
#         return modified_adj
#
...
#             allowed_mask, _ = self.log_likelihood_constraint(modified_adj, ori_adj, ll_cutoff)
#             allowed_mask = allowed_mask.to(self.device)
#             adj_meta_grad = adj_meta_grad * allowed_mask
#         return adj_meta_grad
#
#
#     def inner_train(self, adj):
#         self.surrogate.initialize()
#         self.surrogate.train()
#
#         optimizer = optim.Adam(self.surrogate.parameters(), lr=self.inner_lr, weight_decay=5e-4)
#
#         for i in range(self.inner_epochs):
#             optimizer.zero_grad()
#             output = self.surrogate.forward(self.pyg_data.x, adj)
...
#
#     def get_meta_grad(self,  adj):
#         self.surrogate.eval()
#         output = self.surrogate.forward(self.pyg_data.x, adj)
#         loss_train = F.nll_loss(output[self.train_mask], self.labels[self.train_mask])
#         loss_unlabeled = F.nll_loss(output[self.unlabeled_mask], self.labels[self.unlabeled_mask])
#
#         attack_loss = self.lambda_ * loss_train + (1 - self.lambda_) * loss_unlabeled
...
#             adj_grad = self.get_meta_grad(modified_adj)
#             adj_meta_score = self.get_adj_score(adj_grad, modified_adj, self.ori_adj, self.ll_constraint, self.ll_cutoff)
#             adj_meta_argmax = torch.argmax(adj_meta_score)
#             row_idx, col_idx = utils.unravel_index(adj_meta_argmax, self.ori_adj.shape)
#             self.adj_changes.data[row_idx][col_idx] += (-2 * modified_adj[row_idx][col_idx] + 1)
#             if self.undirected:
#                 self.adj_changes.data[col_idx][row_idx] += (-2 * modified_adj[row_idx][col_idx] + 1)
#
#         self.adj_adversary = self.get_modified_adj().detach()
#
...
class Metattack(BaseMeta):
    def __init__(self, model, nnodes,
                 feature_shape=None, attack_structure=True, attack_features=False,
                 undirected=True, device='cpu', with_bias=False, lambda_=0.5, train_iters=100, lr=0.1, momentum=0.9):
        ...
        for ix, nhid in enumerate(self.hidden_sizes):
            self.weights.append(Parameter(torch.FloatTensor(previous_size, nhid).to(device)))
            self.w_velocities.append(torch.zeros(self.weights[-1].shape).to(device))

            if self.with_bias:
                self.biases.append(Parameter(torch.FloatTensor(nhid).to(device)))
                self.b_velocities.append(torch.zeros(self.biases[-1].shape).to(device))

            previous_size = nhid

        self.weights.append(Parameter(torch.FloatTensor(previous_size, self.nclass).to(device)))
        self.w_velocities.append(torch.zeros(self.weights[-1].shape).to(device))

        if self.with_bias:
            self.biases.append(Parameter(torch.FloatTensor(self.nclass).to(device)))
            self.b_velocities.append(torch.zeros(self.biases[-1].shape).to(device))
        ...
        for j in range(self.train_iters):
            ...
            for ix, w in enumerate(self.weights):
                ...
                if self.sparse_features:
                    hidden = adj_norm @ torch.spmm(hidden, w)
                else:
                    hidden = adj_norm @ hidden @ w
                if self.with_bias:
                    hidden = hidden + self.biases[ix]
    ...
    def get_meta_grad(self, features, adj_norm, idx_train, idx_unlabeled, labels, labels_self_training):
        ...
        if self.attack_features:
            feature_grad = torch.autograd.grad(attack_loss, self.feature_changes, retain_graph=False)[0]

        return adj_grad, feature_grad

    def attack(self,
               ori_features, ori_adj, labels,
               idx_train, idx_unlabeled,
               n_perturbations, ll_constraint=True, ll_cutoff=0.004):
        ...
        for i in tqdm(range(n_perturbations), desc="Perturbing graph"):
            ...
            adj_grad, feature_grad = self.get_meta_grad(modified_features, adj_norm, idx_train, idx_unlabeled, labels, labels_self_training)  # 2925 -> 3087
    ...
    def inner_train(self, features, modified_adj, idx_train, idx_unlabeled, labels, labels_self_training):
        ...
        for j in range(self.train_iters):
            ...
            if self.attack_structure:
                ...
                self.adj_grad_sum += torch.autograd.grad(attack_loss, self.adj_changes, retain_graph=False)[0]
            if self.attack_features:
                self.feature_changes.grad.zero_()
                self.feature_grad_sum += torch.autograd.grad(attack_loss, self.feature_changes, retain_graph=False)[0]
