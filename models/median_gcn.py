...
class MedianGCN(GCN):
    ...
    def __init__(self,
        ...
        super(MedianGCN, self).__init__(config, pyg_data, device, logger, with_relu, with_bias, with_bn)
        self.layers = torch.nn.ModuleList()
        self.layers.append(MedianConv(self.pyg_data.num_features, self.config['num_hidden'], bias=self.with_bias))
        for _ in range(self.num_layers - 2):
            self.layers.append(MedianConv(self.config['num_hidden'], self.config['num_hidden'], bias=self.with_bias))
        self.layers.append(MedianConv(self.config['num_hidden'], self.pyg_data.num_classes, bias=self.with_bias))
